...
(343 por 100.000 hab.), impulsada en parte por la emergencia del robo de veh\'iculo
...
violentos, que reduce el denominador. Los dos tipos de robo que concentran el 93\% del grupo ---el robo con intimidaci\'on y el robo con violencia--- muestran tasas post-pandemia inferiores a su nivel de referencia 2016--2019, mientras la tasa agregada del grupo describe un ciclo: ascenso hasta 2023 e inflexi\'on descendente hacia 2025. La ``crisis'' existe, pero su naturaleza es cualitativa, no cuantitativa.
...
violentos describieron una trayectoria ascendente desde 286 por 100.000
habitantes en 2013 hasta 354 en 2019, colapsaron a 206 durante la pandemia (2021), y
se recuperaron hasta alcanzar un nuevo m\'aximo de 343 en 2023 antes de iniciar una inflexi\'on a la baja en 2024--2025 (283 en 2025). Lo relevante es que, tras el m\'aximo hist\'orico de 2023, el Spline~4 captura una tendencia descendente que el modelo formaliza con IRR\,=\,0.868 ($p=0.057$), convergiendo hacia los niveles del inicio de la serie. Los robos no
violentos muestran un declive secular que abarca todo el per\'iodo: de cerca de 1.657
por 100.000 en 2013 a 1.130 en 2024, sin recuperaci\'on significativa
...
Chile 2013--2025.}
\label{fig:tasas}
\tablenote{La l\'inea horizontal de referencia corresponde al promedio de la l\'inea
base (2016--2019). Fuente: \citet{INE2026}; proyecciones INE Base 2024
(nivel nacional).}
\end{figure}
...
Cuatro modelos de validaci\'on proveen evidencia convergente sobre la especificidad y
...
Un segundo placebo positivo (P3, secuestros) ofrece validaci\'on complementaria. Si el
crimen organizado coercitivo se expandi\'o en el per\'iodo 2022--2025, el modelo debe
registrar crecimiento en los secuestros. El Spline~4 para secuestros arroja
IRR\,=\,1.553 ($p<0.001$), el coeficiente m\'as elevado entre todos los placebos en
el per\'iodo reciente. Este resultado es coherente con el incremento del 263\% en la
retenci\'on de v\'ictimas documentado en la Tabla~\ref{tab:cum} y refuerza la
interpretaci\'on de que el crimen organizado coercitivo s\'i se expandi\'o en dicho
per\'iodo. La coexistencia de una inflexi\'on descendente en robos violentos totales
(IRR\,=\,0.868) con un crecimiento pronunciado en secuestros (IRR\,=\,1.553) confirma
que el modelo discrimina entre distintas formas de violencia patrimonial: el descenso
del volumen agregado no impide detectar fen\'omenos emergentes de alta lesividad.
...
\toprule
 & \multicolumn{3}{c}{\textbf{P1: Cuasidelito vehicular}} & \multicolumn{2}{c}{\textbf{P2: Homicidios CCH}} & \multicolumn{2}{c}{\textbf{P3: Secuestros}} & \multicolumn{2}{c}{\textbf{CPHDV (confirmados)}} \\
\cmidrule(lr){2-4}\cmidrule(lr){5-6}\cmidrule(lr){7-8}\cmidrule(lr){9-10}
\textbf{T\'ermino} & IRR & IC 95\% & $p$ & IRR & $p$ & IRR & $p$ & IRR & $p$ \\
\midrule
...
\end{tabular}
\end{adjustbox}
\tablenote{P1, P2 y P3 se estiman sobre el panel regional (16 regiones $\times$ 156 meses);
...
Inferencia: WCB (pesos de Webb, $B=9{,}999$) para P1, P2 y P3; errores QMLE est\'andar
para CPHDV.}
\end{table}
...
La Tabla~\ref{tab:robustez} reporta el coeficiente del Spline~4 (2022--2025) bajo seis
especificaciones alternativas para robos violentos. Cinco de las seis especificaciones
producen coeficientes negativos, confirmando la inflexi\'on descendente del modelo
principal (IRR\,=\,0.868, $p=0.057$). Las especificaciones R1 (offset libre:
IRR\,=\,0.732, $p=0.010$) y R6 (detenciones en flagrancia: IRR\,=\,0.539, $p<0.001$)
producen declives incluso m\'as pronunciados que el modelo base; R5 (nodos te\'oricos:
IRR\,=\,0.883, $p=0.079$) y R7 (spline m\'as flexible, df\,=\,5: IRR\,=\,0.863,
$p=0.068$) replican la magnitud del resultado principal con coeficientes pr\'acticamente
id\'enticos.

La especificaci\'on R3 (sin correcci\'on SERMIG) constituye la \'unica excepci\'on:
produce un coeficiente pr\'acticamente nulo (IRR\,=\,0.994, $p=0.948$). La exclusi\'on
del corrector migratorio introduce ruido en el denominador de las regiones de alta
inmigraci\'on, atenuando la se\~nal hasta la nulidad; este resultado ilustra la
relevancia emp\'irica de la correcci\'on SERMIG y no contradice la direcci\'on del
hallazgo principal. La especificaci\'on R6, cuyo coeficiente es el m\'as negativo,
indica que las detenciones en flagrancia en el per\'iodo 2022--2025 cayeron incluso
con mayor intensidad que los casos policiales reportados, lo que descarta que la
inflexi\'on descendente sea artefacto de la cifra negra o de cambios en la propensi\'on
a denunciar.
...
\midrule
Principal & Modelo base (spline P25/P50/P75, WCB) & $-$0.141 & 0.868 & .057 \\
\midrule
R1 & Offset libre (pob.\ como regresor)    & $-$0.312 & 0.732 & .010 \\
R3 & Sin correcci\'on SERMIG               & $-$0.006 & 0.994 & .948 \\
R5 & Nodos te\'oricos (2016, 2019, 2022)   & $-$0.125 & 0.883 & .079 \\
R6 & Detenciones como variable dependiente & $-$0.618 & 0.539 & $<$.001 \\
R7 & Spline m\'as flexible (df\,=\,5)      & $-$0.147 & 0.863 & .068 \\
\bottomrule
\end{tabular}
\end{adjustbox}
\tablenote{El estimador reportado corresponde al Spline~4 del modelo (tendencia en el
per\'iodo 2022--2025), que es el t\'ermino de inter\'es principal. Todas las
especificaciones incluyen dummies de estallido/pandemia, efectos fijos regionales y
...
1.15, 1.20\}$ aplicados a regiones de alta inmigraci\'on. El estimador del Spline~4 y el IRR
permanecen id\'enticos en todos los escenarios (estimador\,=\,$-0.141$; IRR\,=\,0.868),
con variaciones m\'inimas en el error est\'andar que no alteran la inferencia (rango
de $p$: 0.048--0.062). La
...
exige respuestas de pol\'itica diferenciadas. El segundo es el nivel de letalidad:
pese al descenso posterior a 2022, los 1.091 homicidios confirmados de 2025
se mantienen sustancialmente por encima de los niveles previos al estallido social.

En este contexto, la trayectoria territorial del crimen organizado en la macrozona Norte
revela una estructura bif\'asica notablemente capturada por los modelos inferenciales y de validaci\'on.
Mientras el incremento de la violencia letal (homicidios dolosos) present\'o all\'i su
fase m\'as expansiva y temprana durante el per\'iodo 2016--2019 (IRR\,=\,2.25), el desborde
de la violencia orientada espec\'ificamente a la coerci\'on patrimonial (robos violentos)
registr\'o sus quiebres estructurales en dicha macrozona reci\'en hacia 2021--2023. Esta
disociaci\'on cronol\'ogica sugiere que la instalaci\'on inicial de redes criminales oper\'o
mediante el control territorial letal en una primera fase, para luego derivar en una
segunda fase de maduraci\'on extractiva sobre el patrimonio ciudadano post-pandemia.
...
Primero, los dos delitos que concentran el 93\% del grupo ---robo con intimidaci\'on
y robo con violencia--- registran tasas post-pandemia inferiores a su nivel de
referencia 2016--2019. La tasa agregada de robos violentos alcanz\'o un m\'aximo
hist\'orico en 2023 e inici\'o una inflexi\'on descendente formalizada en el Spline~4
(IRR\,=\,0.868, $p=0.057$), sugestiva aunque en el l\'imite de la significancia
convencional. El ascenso de la tasa total entre 2020 y 2023 refleja en parte la
